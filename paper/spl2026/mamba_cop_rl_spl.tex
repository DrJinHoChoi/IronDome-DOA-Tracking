...
% =========================================================
\begin{abstract}
We address when temporal memory helps in reinforcement learning
(RL)-based multi-target DOA track management.
We prove that a selective SSM encoder outperforms a memoryless MLP
if and only if the observation is \emph{not} a sufficient statistic
for the policy-relevant history---a condition satisfied by raw
high-dimensional COP spectra over long episodes but not by
hand-crafted low-dimensional features.
We instantiate this as \textbf{Mamba-COP-RL}: a Mamba SSM encoder
coupled with PPO, operating on the 181-dim COP spatial spectrum.
On \textbf{CombatTrackEnv} (four combat scenarios), Mamba-COP-RL
improves GOSPA by up to $25.2\%$ over fixed GM-PHD thresholds and
$6.7\%$ over MLP-PPO, with gains ordered by temporal dependency
horizon as predicted.
The 41.4\,KB INT8 model runs at $3.4$\,ms/scan on STM32H7 Cortex-M7.
\end{abstract}
...
performance in POMDPs; Pleines~\textit{et al.}\ \cite{pleines2022recurrent} analyzed
...
\cite{schafer2007recurrent}, the SSM encoder dominates the MLP in mutual
...
\begin{algorithm}[t]
\caption{Mamba-COP-RL: rollout + PPO update}
\label{alg:mamba_cop_rl}
\small
\begin{algorithmic}[1]
\Require policy $\pi_\phi$, value $V_\phi$, SSM encoder $f_\theta$,
         episode length $T$, PPO epochs $E$
\State initialize $\phi,\theta$; replay buffer $\mathcal{B}\gets\emptyset$
\For{each episode}
  \State $\vec{h}_0 \gets \vec{0}$
  \For{$t=1,\ldots,T$} \Comment{rollout (O$(d_s)$/scan)}
    \State $\mat{X}_t \gets$ array snapshot; compute COP spectrum
    \State $\vec{o}_t \gets [\tilde{P}_{\rm COP}(\theta_{1:181}),\,t/T,\,\rho_{\rm SNR}]^\top$
    \State $\tilde{\vec{o}}_t,\vec{h}_t \gets \textsc{SSM.step}(f_\theta; \vec{o}_t, \vec{h}_{t-1})$ \Comment{Eq.~\ref{eq:encoder}}
    \State $\vec{a}_t \sim \pi_\phi(\cdot \mid \tilde{\vec{o}}_t)$;
           map to $(w_b,\tau_p,p_D)$
    \State run GM-PHD predict/update with new params
    \State $r_t \gets -\mathrm{GOSPA}
           -0.01|\hat K_t-K_t^*|
           -0.001\max(0,J_t-50)$
    \State store $(\tilde{\vec{o}}_t,\vec{a}_t,r_t,V_\phi(\tilde{\vec{o}}_t))$ in $\mathcal{B}$
  \EndFor
  \State compute GAE advantages $\hat A_{1:T}$; returns $\hat R_{1:T}$
  \For{$e=1,\ldots,E$} \Comment{PPO + BPTT}
    \State $\tilde{\vec{o}}_{1:T} \gets \textsc{SSM.forward}(f_\theta; \vec{o}_{1:T}, \vec{h}_0{=}\vec{0})$
    \State update $\phi,\theta$ on clipped surrogate
           $+\;\beta_V(\hat R-V_\phi)^2 - \beta_H\mathcal{H}[\pi_\phi]$
  \EndFor
\EndFor
\State \Return $\pi_\phi, V_\phi, f_\theta$
\end{algorithmic}
\end{algorithm}
...
\begin{figure}[!t]
  ...
  \caption{Stealth scenario ($K{=}3$, $T{=}200$, $M{=}8$).
  \emph{Left}: DOA trajectories (Target~3 invisible during scans
  60--105, yellow band; Mamba re-acquires in $+3$ vs.\ $+15$ for
  fixed threshold). \emph{Right}: Per-scan GOSPA (window$=10$).
  Mean GOSPA: Fixed $0.250$, MLP $0.201$, Mamba $\mathbf{0.187}$.}
  \label{fig:tracking}
\end{figure}

Fig.~\ref{fig:tracking} shows $\vec{h}_t$ retaining the trajectory
through the 45-scan visibility gap: Mamba re-associates in $+3$ scans
vs.\ $+15$ (fixed) and $+8$ (MLP-PPO).
