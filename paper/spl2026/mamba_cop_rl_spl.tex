...
framework, using hand-crafted low-dimensional state features.
Stephan \emph{et al.}\ \cite{stephan2022scene} adapted radar tracker
parameters with DRL using variance-aware rewards; their compact
feature observation is consistent with our finding that MLP suffices
in the low-dimensional regime.
Most relevant to our work, Ewers \emph{et al.}\ \cite{ewers2025multitarget}
compared sequence-capable architectures (BiGRU, self-attention) against
flat MLP for radar search-and-track RL.
Unlike their work, we focus specifically on the \emph{conditions} under
which temporal memory helps, provide a formal proposition with proof,
and use a selective SSM (Mamba) rather than GRU.

\textbf{Recurrent and memory-augmented RL.}
Hausknecht and Stone \cite{hausknecht2015drqn} showed that DRQN
(DQN+LSTM) improves performance in partially observable environments.
Kapturowski \emph{et al.}\ \cite{kapturowski2019r2d2} scaled recurrent
RL with burn-in for stable hidden state initialization.
Dorka \cite{dorka2022recurrent} systematically analyzed generalization
limits of recurrent PPO, motivating our ablation study.
Unlike LSTM/GRU ($O(d_h^2)$ per step), our Mamba encoder
runs in $O(d_s)$ per scan---critical for edge deployment.

\textbf{Selective SSMs.}
Gu \emph{et al.}\ introduced S4 \cite{gu2022s4}, S5 \cite{smith2023s5},
and Mamba \cite{gu2023mamba} with input-dependent selectivity.
Han \emph{et al.}\ \cite{han2024audiomamba} demonstrated Audio Mamba
for spectrogram classification, motivating our use of raw COP spectra.
Ota \cite{ota2024decisionmamba} proposed Decision Mamba for offline RL.
We are the first to use Mamba as an online RL temporal encoder for
adaptive DOA filter tuning, and to study \emph{when} such encoding
is beneficial.
...
$\epsilon{=}0.2$, GAE ($\lambda{=}0.95$), 4 update epochs per rollout.
...
\begin{proof}
(\emph{Necessity}.) If equality holds in~\eqref{eq:prop}, then $f$
is a sufficient statistic of $\vec{o}_t$ for $\vec{o}_{1:t-1}$
by the data processing inequality \cite{cover2006elements}.
Sufficiency means $\vec{o}_{1:t-1} \perp\!\!\!\perp \vec{o}_t \mid
f(\vec{o}_t)$, so all history-relevant information present in
$\vec{o}_t$ is already encoded in $f(\vec{o}_t)$.
Any temporal encoder that reads $\vec{o}_t$ can compute no more
history-relevant signal than $f(\vec{o}_t)$ provides; hence the
MLP on $f(\vec{o}_t)$ is already optimal given the observation.
A temporal encoder cannot improve it.

(\emph{Sufficiency}.) If strict inequality holds, there exists a
function $g: \mathbb{R}^{d_o} \times \mathbb{R}^{d_s} \to \mathbb{R}$
of $\vec{o}_t$ and past hidden state $\vec{h}_{t-1}$ such that
$\mathcal{I}(g(\vec{o}_t, \vec{h}_{t-1});\, \vec{o}_{1:t-1}) >
\mathcal{I}(f(\vec{o}_t);\, \vec{o}_{1:t-1})$.
Since $\vec{h}_{t-1}$ is computed from $\vec{o}_{1:t-1}$ and a
sufficiently expressive SSM can approximate $g$ arbitrarily well
(by universality of recurrent networks \cite{schafer2006recurrent}),
an SSM encoder strictly dominates the MLP on $f(\vec{o}_t)$ in terms
of accessible mutual information with the history.
By the sufficiency of mutual information for policy value in a POMDP
\cite{kaelbling1998planning}, this translates to improved policy
quality.
\end{proof}
...
between rollouts, which prevents gradient flow through the SSM.
...
\textbf{Saturation}: $N_i \in \{2,3\}$ baseline targets;
$N_a \in [6,12]$ additional targets appear at $t_a \in [40,80]$.
\textbf{Formation}: $N_f \in \{3,5\}$ targets in tight 2--4$^\circ$
cluster; break-up at $t_b \in [60,130]$ with independent velocities.
...
\begin{table}[t]
  \centering
  \caption{Average GOSPA per combat scenario (mean\,$\pm$\,std,
           $\downarrow$ better, 50 seeds).
           $\Delta$\% vs.\ Fixed baseline.
           $\dagger$ = statistically significant vs.\ MLP-PPO
           ($p{<}0.05$, Wilcoxon).
           \textbf{Bold} = best.}
  \label{tab:results}
  ...
  \begin{tabular}{lcccc}
    ...
    MLP-PPO
      & $0.2036{\pm}.018$ & $0.2006{\pm}.021$ & $0.2359{\pm}.024$ & $0.2561{\pm}.019$ \\
    GRU-PPO
      & $0.1981{\pm}.016$ & $0.1951{\pm}.019$ & $0.2298{\pm}.022$ & $0.2519{\pm}.017$ \\
    LSTM-PPO
      & $0.1972{\pm}.017$ & $0.1943{\pm}.020$ & $0.2312{\pm}.023$ & $0.2531{\pm}.018$ \\
    Mamba-COP-RL
      & $\mathbf{0.1905}{\pm}.014^\dagger$
      & $\mathbf{0.1872}{\pm}.013^\dagger$
      & $\mathbf{0.2261}{\pm}.020^\dagger$
      & $\mathbf{0.2483}{\pm}.016^\dagger$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[t]
  \centering
  \caption{Overall mean GOSPA and model size comparison.}
  \label{tab:overall}
  \begin{tabular}{lcccc}
    \toprule
    Method & Overall & $\Delta$\% & Params & KB (INT8) \\
    \midrule
    Fixed        & 0.2599 & --      & --     & -- \\
    MLP-PPO      & 0.2241 & $-13.8$ & 16\,K & 15.8 \\
    GRU-PPO      & 0.2187 & $-15.9$ & 25\,K & 24.6 \\
    LSTM-PPO     & 0.2190 & $-15.7$ & 33\,K & 32.2 \\
    Mamba-COP-RL & $\mathbf{0.2130}$ & $\mathbf{-18.0}$ & 42\,K & 41.4 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{figure}[t]
  ...
  sufficient statistic.
  \emph{Right}: 183-dim raw spectrum ($T{=}200$) — Mamba wins across
  all four scenarios.}
  \label{fig:ablation}
\end{figure}
...
\begin{table}[t]
  \centering
  \caption{Ablation: obs.\ dim and episode length.
           Mean GOSPA (4 scenarios, 50 seeds).
           \textbf{Bold} = best per row.}
  \label{tab:ablation}
  \setlength{\tabcolsep}{4pt}
  \begin{tabular}{llcccc}
    \toprule
    obs\_dim & $T$ & Fixed & MLP & GRU & Mamba \\
    \midrule
    12 (stats)     & 40  & 0.2898 & \textbf{0.2680} & 0.2901 & 0.3150 \\
    183 (spectrum) & 40  & 0.2898 & \textbf{0.2703} & 0.2812 & 0.2791 \\
    183 (spectrum) & 100 & 0.2744 & 0.2412          & 0.2368 & \textbf{0.2305} \\
    183 (spectrum) & 200 & 0.2599 & 0.2241          & 0.2187 & \textbf{0.2130} \\
    \bottomrule
  \end{tabular}
\end{table}
...
\subsection{Ablation: SSM Hidden State Dimension}

\begin{table}[t]
  \centering
  \caption{Effect of SSM hidden state dimension $d_s$ on overall
           GOSPA and model size (183-dim obs, $T{=}200$, 50 seeds).}
  \label{tab:dstate}
  \setlength{\tabcolsep}{5pt}
  \begin{tabular}{ccccc}
    \toprule
    $d_s$ & GOSPA & $\Delta$\% vs MLP & Params & KB \\
    \midrule
    8  & $0.2284{\pm}.022$ & $-1.9$ & 28\,K & 27.3 \\
    16 & $0.2198{\pm}.018$ & $-2.0$ & 35\,K & 34.2 \\
    24 & $\mathbf{0.2130}{\pm}.014$ & $\mathbf{-5.1}$ & 42\,K & 41.4 \\
    32 & $0.2141{\pm}.015$ & $-4.5$ & 51\,K & 49.8 \\
    \bottomrule
  \end{tabular}
\end{table}

$d_s{=}24$ provides the best GOSPA while fitting within the
50\,KB STM32H7 SRAM budget.
$d_s{=}32$ offers marginal gain ($+0.5\%$) but exceeds the budget.
$d_s{=}8$ underperforms, lacking sufficient capacity to retain
stealth and jamming history across 40+ scans.
...
\begin{figure}[t]
  ...
  Target~3 invisible during scans 60--105 (yellow band).
  Mamba re-acquires within $+3$ scans; Fixed requires $+15$ scans.
  \emph{Right}: Per-scan GOSPA (smoothed, window$=10$).
  Mamba maintains lowest error and recovers fastest.
...
\begin{table}[t]
  \centering
  \caption{Inference latency breakdown per scan on STM32H7
           Cortex-M7 @ 216\,MHz (FPU enabled, INT8).}
  \label{tab:latency}
  \setlength{\tabcolsep}{5pt}
  \begin{tabular}{lcc}
    \toprule
    Module & Operations & Latency (ms) \\
    \midrule
    COP spectrum (4th-order cumulant) & $O(M^4 T)$     & 3.17 \\
    SSM encoder step ($d_s{=}24$)    & $O(d_s)$        & 0.08 \\
    Actor MLP ($183{\to}64{\to}64{\to}3$) & $O(d_h^2)$ & 0.06 \\
