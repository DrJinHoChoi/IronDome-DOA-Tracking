...
When the number of sources $K$ approaches or exceeds the single-stage COP capacity $\rho(M-1)$, estimation performance degrades due to the exhaustion of virtual degrees of freedom. SD-COP addresses this by borrowing the successive interference cancellation (SIC) concept from communications~\cite{andrews2005sic}: after estimating the strongest sources via COP, their contributions are subtracted from the original received signal in the signal domain via joint least-squares reconstruction, and the cumulant is recomputed from the residual. This signal-domain deflation avoids the cumulant power estimation errors inherent in direct cumulant-domain subtraction. A multi-pass global refinement with leave-one-out spectrum computation further improves accuracy, extending the effective capacity well beyond $\rho(M-1)$.
...
The multi-target DOA tracking problem is naturally modeled using random finite sets (RFS)~\cite{mahler2003phd,mahler2007statistical}. Extensions such as the cardinalized PHD (CPHD)~\cite{vo2013labeled} further improve cardinality estimation. At scan $n$, the multi-target state is the RFS
\begin{equation}
...
\begin{enumerate}
\item The COP family resolves $K > M-1$ sources using only $M$ sensors. We experimentally confirm that COP resolves all $K = \rho(M-1) = 14$ sources at the theoretical capacity limit (mean error $0.54^\circ$), while SD-COP and T-COP extend operation to $K > \rho(M-1)$ through sequential deflation and temporal accumulation, respectively.
...
The proposed framework is particularly relevant to applications where multiple targets must be simultaneously detected, localized, and tracked using compact sensor arrays with limited aperture. Future work includes extension to two-dimensional (azimuth--elevation) DOA estimation, non-uniform array geometries, evaluation using the GOSPA metric~\cite{rahmathullah2017gospa} for comprehensive multi-target performance assessment, and real-time implementation on embedded radar platforms.
