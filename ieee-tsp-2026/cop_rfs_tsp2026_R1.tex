...
\textbf{Physics-based motion makes the SOTA back-end best under maneuver.}
...
scenario (SNR $6$~dB, Table~\ref{tab:maneuver}) it cuts $\delta$-GLMB's
localization error by $45\%$ ($1.21^\circ\!\to\!0.67^\circ$) and switches sevenfold
($13.1\!\to\!1.8$). Tellingly, \emph{CV} $\delta$-GLMB trails even the CV TO-PHD
here---its joint association misled by lagging predictions---so the physics model
is what makes the SOTA back-end robust under maneuver. Our recommended deployed
configuration is therefore the \emph{gated closed-loop higher-order front-end
driving a physics-based, interchangeable SOTA back-end} (instantiated
with $\delta$-GLMB): agnostic in both estimator and tracker, gated for safety, and
matched to the target dynamics.

\begin{table}[t]
\centering
\caption{Physics-based motion model for \emph{maneuvering} targets ($K=4$ sinusoidal maneuver, SNR $6$~dB, $30$ trials, $95\%$ CIs). The constant-acceleration (CA) physics makes the SOTA $\delta$-GLMB best-in-class on every metric; CV $\delta$-GLMB is misled by its lagging predictions and trails even the CV TO-PHD.}
\label{tab:maneuver}
\small
\resizebox{\columnwidth}{!}{%
\begin{tabular}{@{}lcccc@{}}
...
$\delta$-GLMB (CA, physics) & $\mathbf{97.5\pm1.1}$ & $\mathbf{0.67\pm0.06}$ & $\mathbf{1.62\pm0.23}$ & $\mathbf{1.8\pm1.0}$ \\
\bottomrule
\end{tabular}}
\end{table}
