...
configuration}---the destination of this paper---is a gated closed-loop COP
front-end driving a \emph{physics-based}, interchangeable SOTA back-end
($\delta$-GLMB), robust across source count, SNR, motion, and
...
open-loop estimate. The recommended configuration is a gated COP
front-end driving a physics-based, interchangeable SOTA back-end (here $\delta$-GLMB).}
\label{fig:system}
\end{figure*}
...
Fig.~\ref{fig:pdk} sweeps the \emph{front-end} detection rate over $K$ (no
tracker): the two methods are indistinguishable for $K\le M{-}1=7$, but at $K=8$
MUSIC collapses ($100\%\!\to\!49\%$) toward its $(M{-}1)/K$ ceiling while COP
degrades only gradually toward the virtual-array limit $\rho(M{-}1)=14$---the
structural signature of the extended aperture, not parameter tuning. (The raw
front-end MUSIC rate at $K=10$, ${\sim}25\%$, is below the $52\%$ end-to-end of
Table~\ref{tab:underdet}: the tracker recovers some detections by temporal
integration but cannot resolve structurally-lost sources.)

\begin{figure}[t]
\centering
\includegraphics[width=0.8\linewidth]{figures/fig_pd_vs_k.png}
...
$K\in\{2,\dots,12\}$: in the determined regime ($K\le M{-}1$) all methods detect
every source, but past $K{=}7$ MUSIC's detection collapses toward its $(M{-}1)/K$
cap while COP---and, most strongly, the proposed gated closed loop---degrade
gracefully (at $K{=}10$, $P_d=97.8\%$ for the gated loop vs.\ $79.0\%$ open-loop
COP vs.\ $25.5\%$ MUSIC). Fig.~\ref{fig:robustness}(bottom) fixes $K=10>M{-}1$ and
...
slowly varying targets ($K=6$). Fig.~\ref{fig:loop2x2} and Table~\ref{tab:loop2x2}
show the tracker-aided refinement \emph{lowers the localization error of both
...
\begin{figure}[t]
\centering
\includegraphics[width=0.76\linewidth]{figures/fig_loop_2x2.png}
...
practical system attains the gain.
