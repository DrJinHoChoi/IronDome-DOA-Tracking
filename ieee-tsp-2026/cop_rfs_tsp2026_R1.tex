...
fed back---through a two-stage validation gate and a Temporal-COP (T-COP) subspace
refinement---to sharpen the estimator, closing the loop. The framework is
deliberately \emph{modular}: it is \emph{front-end-agnostic} (instantiated with the
higher-order-cumulant COP front-end, yielding COP-RFS for the underdetermined
regime $K>M{-}1$, and with a covariance MUSIC front-end) and
\emph{back-end-agnostic} (PHD, LMB, or $\delta$-GLMB trackers). Its
\emph{recommended deployed configuration}---the destination of this paper---is a
gated closed-loop COP front-end driving a \emph{physics-based} $\delta$-GLMB
back-end, which we show is robust across source count, SNR, target motion, and
signal type. The remainder develops and validates each component of this system;
