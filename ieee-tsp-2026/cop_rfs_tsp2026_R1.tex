...
no-harm guarantee from the front-end to the \emph{complete system}. A summary of
the operating regimes is given in Table~\ref{tab:regime}.
...
$M_v{-}1$, whereas the closed-loop PCRB~\eqref{eq:pcrb} stays bounded by the prior
information $1/\lambda_{\min}(\mat{J}_{k|k-1})$, confirming that the temporal prior
restores identifiability; in the well-determined regime the two bounds coincide.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig_pcrb_validation.png}
\caption{Validation of Theorem~1: the open-loop bound diverges at the
virtual-array limit $K{=}M_v{-}1$ while the closed-loop PCRB stays finite, bounded
by the prior information; near-parity in the well-determined regime.}
\label{fig:pcrb}
\end{figure}

\textbf{Minimum-variance refinement (Theorem~2).}
Fig.~\ref{fig:tcopmse} validates~\eqref{eq:tcopB}--\eqref{eq:tcop_bias}: the fused
subspace MSE matches the predicted factor $1/(1+\kappa)$ for an unbiased prior, and
the biased-case break-even lands at $\|\mat{b}\|^2=D(\sigma_e^2+\sigma_p^2)$, with
the validation gate keeping the fused MSE at or below the data-only MSE.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig_tcop_mse_validation.png}
\caption{Validation of Theorem~2: (top) unbiased-prior MSE follows $1/(1+\kappa)$;
(bottom) biased-prior break-even at $\|\mat{b}\|^2=D(\sigma_e^2+\sigma_p^2)$, where
the gated estimator clamps to and never exceeds the data-only MSE.}
\label{fig:tcopmse}
\end{figure}
