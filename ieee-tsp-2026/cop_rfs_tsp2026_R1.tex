...
\begin{abstract}
This paper presents a closed-loop subspace--RFS framework that couples direction-of-arrival (DOA) estimation with random-finite-set (RFS) multi-target tracking and, unlike prior open-loop cascades, analyzes the coupling itself. The closed loop is front-end-agnostic---applicable to any subspace DOA estimator---and we instantiate it with both a higher-order-cumulant front-end (COP, which extends operation to the underdetermined regime $K>M{-}1$; the resulting tracker is termed COP-RFS) and a covariance front-end (MUSIC). Building on the higher-order-cumulant Constrained Optimization of the Pseudo-spectrum (COP) algorithm~\cite{choi2015underdetermined}, which uses a virtual array of size $M_v=\rho(M{-}1){+}1$ to resolve up to $\rho(M{-}1)$ sources from $M$ sensors, the tracker's temporal prediction is fed back to a Temporal COP (T-COP) stage that refines the cumulant signal subspace. Our central contributions are theoretical. First, we derive a posterior Cram\'{e}r--Rao bound for the \emph{complete closed-loop} estimator--tracker and prove that the temporal prior restores identifiability and strictly tightens the bound precisely in the underdetermined regime where the data-only bound diverges. Second, we recast the T-COP refinement as a basis-invariant inverse-variance fusion on the Grassmann manifold and prove it is minimum-variance, reducing estimation error under an explicit prior-accuracy condition that a validation gate enforces, so that the closed loop never degrades performance. The back-end is a physics-based track-oriented PHD (TO-PHD) filter whose predict--identify--update pipeline preserves track identity through crossings; the framework is equally back-end-agnostic, which we confirm by also integrating labeled multi-Bernoulli (LMB) and full $\delta$-GLMB filters. Monte-Carlo experiments (up to $500$ trials, with $95\%$ confidence intervals) show that COP-RFS detects significantly more targets than a MUSIC--PHD baseline in the underdetermined regime ($K>M{-}1$); that the closed-loop refinement improves \emph{both} the cumulant (COP) and covariance (MUSIC) front-ends, confirming it as a front-end-agnostic principle; and that the gated closed loop reduces localization error by up to $56\%$ (COP) and $44\%$ (MUSIC) in the low-SNR regime predicted by the theory while matching the open-loop baseline otherwise, its benefit for moving targets manifesting as a $15\%$ reduction in identity switches at matched detection. Rather than claiming uniform superiority, we explicitly delineate the operating regime in which the higher-order front-end is advantageous.
\end{abstract}
