% A Closed-Loop Subspace-RFS Framework for Multi-Target DOA Tracking -- IEEE TSP 2026
...
\begin{abstract}
We present a closed-loop subspace--RFS framework that couples direction-of-arrival (DOA) estimation with random-finite-set (RFS) multi-target tracking and, unlike prior open-loop cascades, analyzes the coupling itself. The loop is \emph{front-end-agnostic}---the tracker's temporal prediction is fed back to refine the signal subspace of any subspace DOA estimator---and we instantiate it with a higher-order-cumulant front-end (COP, which reaches the underdetermined regime $K>M{-}1$; the resulting tracker is COP-RFS) and a covariance front-end (MUSIC). Our central contributions are theoretical: (i)~a posterior Cram\'{e}r--Rao bound for the \emph{complete closed-loop} estimator--tracker, proving the temporal prior restores identifiability exactly where the data-only bound diverges; and (ii)~a recasting of the subspace refinement as minimum-variance Grassmann fusion, with a validation gate guaranteeing the loop never degrades performance. The framework is also \emph{back-end-agnostic} (track-oriented PHD, LMB, $\delta$-GLMB); a physics-based constant-acceleration model makes $\delta$-GLMB best under maneuver, giving our recommended configuration: a gated closed-loop front-end (COP, or lower-variance MUSIC when determined) driving a physics-based, interchangeable SOTA back-end ($\delta$-GLMB). In Monte-Carlo experiments (up to $500$ trials, $95\%$ CIs), COP-RFS detects significantly more targets than a MUSIC--PHD baseline when $K>M{-}1$, and the gated loop lowers low-SNR localization error by up to $56\%$ (COP) and $44\%$ (MUSIC) and cuts moving-target identity switches by $15\%$, matching the open-loop baseline otherwise---gains we delineate by operating regime rather than claiming uniform superiority.
\end{abstract}
...
We derive the first Cram\'{e}r--Rao bound for the coupled higher-order-cumulant/RFS tracker (Theorem~1). The posterior-CRB recursion itself is classical~\cite{tichavsky1998posterior}; our new result is the \emph{identifiability phase transition} it exposes in the underdetermined cumulant regime---as the cumulant information turns rank-deficient the data-only bound \emph{diverges}, while the closed-loop bound stays \emph{finite}, capped by the temporal prior. This is a qualitative statement that no static or data-only CRLB can make, and it replaces the prior version's unsupported ``exact'' static-cumulant CRLB.

\textbf{Contribution 2 (Theory): Provably minimum-variance T-COP refinement.}
We prove the tracker-aided T-COP refinement is a minimum-variance fusion on the Grassmann manifold (Theorem~2). Inverse-variance fusion is classical; what is new is that we make it \emph{gauge-invariant}---via Procrustes alignment and the projector $\mat{P}_s$, curing the basis-dependence of the prior version's subspace interpolation---and pair it with an \emph{explicit, checkable} benefit condition $\|\mat{b}\|^2<D(\sigma_e^2+\sigma_p^2)$ that a validation gate \emph{enforces online}. The closed loop therefore \emph{provably never degrades} the open-loop estimate: it strictly improves it when the prior is accurate (low SNR, slow targets) and falls back to open-loop COP otherwise.

\textbf{Contribution 3 (Practice): Underdetermined capability and honest evaluation.}
Through Monte-Carlo experiments (up to $500$ trials, $95\%$ CIs) against MUSIC, closed-loop MUSIC, LMB, and $\delta$-GLMB baselines, we show COP-RFS detects significantly more targets than the MUSIC--PHD cascade when $K>M{-}1$, and we \emph{delineate the operating regime} in which the higher-order front-end is advantageous rather than claiming uniform superiority. The framework is both \emph{front-end-agnostic}---a $2\times2$ (front-end~$\times$~loop) study shows the closed-loop refinement improves \emph{both} COP and MUSIC---and \emph{back-end-agnostic}: in a tracking ablation the labeled TO-PHD, LMB, and $\delta$-GLMB trackers each cut identity switches by an order of magnitude over the unlabeled GM-PHD, with $\delta$-GLMB giving the cleanest identity and TO-PHD a lightweight alternative. A physics-based constant-acceleration (CA) motion model sharpens \emph{every} back-end under maneuver ($\delta$-GLMB best-in-class), yielding our recommended configuration: a gated closed-loop front-end driving a physics-based, interchangeable SOTA back-end ($\delta$-GLMB). Sequential-deflation COP (SD-COP) is a supporting mechanism for the super-underdetermined regime.
...
trackers requested in review. Table~\ref{tab:ablation} shows all three
...
The contributions are threefold. \emph{(i)}~A posterior CRB for the complete coupled estimator--tracker (Theorem~1) proves the temporal prior strictly tightens the bound and restores identifiability exactly in the underdetermined regime where the data-only bound diverges. \emph{(ii)}~The T-COP refinement is recast as a basis-invariant, minimum-variance Grassmann inverse-variance fusion (Theorem~2) under an explicit prior-accuracy condition that a validation gate enforces, so the loop improves estimation when the prior is accurate and never degrades it otherwise. \emph{(iii)}~A CI-backed Monte-Carlo evaluation (up to $500$ trials) against MUSIC, closed-loop MUSIC, LMB, and $\delta$-GLMB baselines establishes the loop as both front-end- and back-end-agnostic---COP-RFS detecting more targets than the MUSIC--PHD cascade when $K>M{-}1$---with the operating regime (low SNR / slow motion for the estimation gain, $K>M{-}1$ for the detection gain) explicitly delineated rather than uniform superiority claimed; SD-COP extends operation toward the super-underdetermined regime.
