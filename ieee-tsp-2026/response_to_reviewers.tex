...
\item \textbf{New title and framing}: ``A Closed-Loop Subspace--RFS Framework
  for Multi-Target DOA Tracking.'' The closed-loop refinement is shown to be
...
(Sec.~VI) that did not exist in the prior version. We further show the closed loop
...
stated as two theory results plus one practical capability. To preempt the
concern that these reduce to known results, each theorem now \emph{explicitly
demarcates} the classical machinery it builds on (the
Tichavsk\'y--Muravchik--Nehorai posterior-CRB recursion; best-linear-unbiased
estimation) from the new content it yields---an identifiability phase transition
in the underdetermined cumulant regime (Theorem~1, where the data-only bound
\emph{diverges} while the closed-loop bound stays finite) and a gauge-invariant,
gate-enforceable minimum-variance subspace fusion (Theorem~2)---neither of which a
static CRB or a heuristic closed loop can provide.
...
velocity-gated merge is corrected (Sec.~IV-D) and its role is tied to Theorem~2's
...
estimator--tracker, validated against Monte-Carlo simulation (Sec.~VII).
...
MUSIC--PHD cascade (Sec.~VII). All three labeled trackers (our TO-PHD, LMB, and
...
\reply We chose the latter---a full-length TSP paper with strengthened theory. The paper
is reorganized around the two theorems and the underdetermined capability;
lightly-justified mechanisms and redundant experiments were compressed and the
novelty positioning rewritten (Abstract, Sec.~I), while the two new theorems, the
SOTA labeled-RFS ($\delta$-GLMB/LMB) baselines, and the expanded Monte-Carlo
evaluation account for the length. The result is a focused $12$-page paper
(two-column IEEE format), within the regular-paper length for the Transactions.
...
implementation of Vo--Vo--Hoang 2017) (Sec.~VII back-end ablation, Table on
...
full-state Mahalanobis term would double-count the velocity (Sec.~IV-D).
...
inconsistency is removed (Sec.~VII).
